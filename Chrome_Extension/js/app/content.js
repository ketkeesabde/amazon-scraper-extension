...
if(url.includes('amazon.in/gp/css/order-history')){
    ...
    setTimeout(function(){ 
	    window.location.href = 'https://www.amazon.in/gp/your-account/order-history?orderFilter='+purchaseYears.slice(-1)[0]+'&ahf=on'; 
    	}, 
    10000);
} else if (url.includes('amazon.in/gp/your-account/') && url.includes('&ahf=on') && url.includes('orderFilter=')){
            ...
            function(response){
                ...
                if(eventName=='ordersPageDetails'){
                    if(response.nextWhat == 'nextYear'){
                      window.location.href = 'https://www.amazon.in/gp/your-account/order-history?orderFilter=year-'+response.year+'&ahf=on';
                    } else if (response.nextWhat == 'nextPage' && typeof response.year != 'undefined'){
                      window.location.href = 'https://www.amazon.in/gp/your-account/order-history/ref=ppx_yo_dt_b_pagination_1_2_3_4_5?ie=UTF8&orderFilter=year-'+response.year+'&search=&startIndex='+response.startIndex+'&ahf=on';
                    }
                } 
            }
    );
...
function fetchSummaryAndReviews(product){
    ajaxGet(product.product_link.split('amazon.in')[1], function(response){
